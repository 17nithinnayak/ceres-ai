...
const Dashboard = () => {
  // State to hold user's farm coordinates
  const [userCoords, setUserCoords] = useState({ lat: 12.0, lng: 75.0 }); // default coords

  useEffect(() => {
    // Load farm location from localStorage
    const storedLocation = localStorage.getItem("farmLocation"); // format: "lat,lng"
    if (storedLocation) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-950 to-black text-gray-200 font-inter">
      <main className="max-w-7xl mx-auto px-6 py-16">
        ...
        <motion.div
          ...
          className="mb-12"
        >
          <DiseaseRiskWidget userLocation={`${userCoords.lat}, ${userCoords.lng}`} />
        </motion.div>
        ...
        <div className="grid md:grid-cols-4 gap-8 mb-12">
          {stats.map((s, i) => (
            <motion.div
              ...
              className="bg-gray-800 rounded-2xl shadow-xl p-6 text-center border border-gray-700 hover:shadow-green-500/30 hover:scale-[1.02] transition-all"
            >
              <h2 className="text-4xl font-bold text-green-400 mb-2">{s.value}</h2>
              <p className="text-gray-400 font-medium">{s.label}</p>
              <p className={`text-sm font-semibold mt-2 ${s.change.includes("+") ? "text-green-400" : "text-red-400"}`}>
                {s.change}
              </p>
            </motion.div>
          ))}
        ...
        <div className="grid md:grid-cols-3 gap-10">
          ...
          <div className="md:col-span-2 order-2 md:order-1">
            <motion.div
              ...
              className="mb-8"
            >
              <CoorgDiseaseMap userCoords={userCoords} />
            </motion.div>

            {/* Recent Scans */}
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-white">Latest Farm Scans</h3>
              <a href="/history" className="text-green-400 font-medium hover:underline">
                View All
              </a>
            </div>
            <div className="space-y-4">
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className={`w-10 h-10 flex items-center justify-center rounded-full ${
                      scan.color === "green" ? "bg-green-900/50 text-green-300" : "bg-red-900/50 text-red-300"
                    }`}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 2a10 10 0 100 20 10 10 0 000-20z" />
                      </svg>
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    scan.color === "green" ? "bg-green-700/30 text-green-300" : "bg-red-700/30 text-red-300"
                  }`}>
                    {scan.status}
                  </span>
                </motion.div>
          ...
          >
            <h3 className="text-xl font-bold mb-4 text-white">Quick Actions</h3>
            <button className="w-full bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg shadow-lg shadow-green-600/30 mb-3 transition-transform hover:scale-[1.02]  " onClick={() => Navigate("/scan")}>
              New Scan: Coffee/Pepper Leaf
            </button>
            <button className="w-full border border-green-600 text-green-400 hover:bg-green-900/50 font-semibold py-3 rounded-lg transition-colors" onClick={() => Navigate("/history")}>
              View Full History
            </button>
